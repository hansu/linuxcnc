...
Or an other user has an MPG Wheel with 500 ipr and wants to set the spindle override which has
...
* material: A modern icon theme inspired by Google's Material Icons that automatically adopts its coloring from the selected deskop theme
...
That way, icon themes can be created that work well with dark and also light desktop themes (in fact, thats not always the best option, thats why a dedicated "material-light" theme exists).
